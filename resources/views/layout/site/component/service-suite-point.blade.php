<div class="alert alert-info text-center {{ Browser::isMobile() ? 'fs-6' : 'fs-4' }}">
    {!! __('site.wording.you_like_add_additional_points') !!}
</div>

<div class="text-center">
    <a href="{{ route('Site-ServiceGetDetail',['service'=> collect($service_list)->where('slug', '=', 'point')->first()['uuid']]) }}"
        class="btn btn-warning m-2">
        <i class="mdi mdi-plus-circle"></i>
        {{ __('site.label.point') }}s
        <hr class="m-1">
        <span class="fs-6">{{ __('site.wording.add_additional_points') }}</span>
    </a>
